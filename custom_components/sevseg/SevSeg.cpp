...
float SevSeg::get_setup_priority() const { return setup_priority::HARDWARE; }
void SevSeg::setup() {
  ESP_LOGCONFIG(TAG, "Setting up SevSeg...");

  byte numDigits = 2;
  byte digitPins[] = {23, 2};
  byte segmentPins[] = {19, 18, 15, 5, 4, 21, 22};
  ...
  begin(hardwareConfig, numDigits, digitPins, segmentPins, resistorsOnSegments, updateWithDelays, leadingZeros, disableDecPoint);
  setBrightness(0);

  // sevseg.setChars("bo");
  byte bo[] = {B01111100, B01011100};
  blank();
  setSegments(bo);

  startTimer();
}
void SevSeg::dump_config() {
  ESP_LOGCONFIG(TAG, "SevSeg:");
...
SevSeg::SevSeg() {
  if (seg) {
    ESP_LOGCONFIG(TAG, "SevSeg already constructed!");
    return;
  }
  ESP_LOGCONFIG(TAG, "Constructing SevSeg...");
...
void IRAM_ATTR SevSeg::refreshDisplay() {
...
void IRAM_ATTR SevSeg::segmentOn(byte segmentNum) {
...
void IRAM_ATTR SevSeg::segmentOff(byte segmentNum) {
  for (byte digitNum = 0 ; digitNum < numDigits ; digitNum++) {
...
// (according to digitCodes[])
void IRAM_ATTR SevSeg::digitOn(byte digitNum) {
  digitalWrite(digitPins[digitNum], digitOnVal);
  for (byte segmentNum = 0 ; segmentNum < numSegments ; segmentNum++) {
    if (digitCodes[digitNum] & (1 << segmentNum)) { // Check a single bit
      digitalWrite(segmentPins[segmentNum], segmentOnVal);
...
void IRAM_ATTR SevSeg::digitOff(byte digitNum) {
  for (byte segmentNum = 0 ; segmentNum < numSegments ; segmentNum++) {
    digitalWrite(segmentPins[segmentNum], segmentOffVal);
  }
  digitalWrite(digitPins[digitNum], digitOffVal);
...
void SevSeg::setChars(const char str[]) {
  ESP_LOGCONFIG(TAG, "setChars: %s", str);
...
void SevSeg::printf(const char *format, ...) {
  va_list arg;
  va_start(arg, format);
  char buffer[64];
